...
#define CUDACHECK(cmd) do { \
  cudaError_t e = (cmd); \
  if (e != cudaSuccess) { \
    fprintf(stderr, "rank %d CUDA error %s:%d %s\n", gRank, __FILE__, __LINE__, cudaGetErrorString(e)); \
    return 1; \
  } \
} while (0)
...
int main(int argc, char** argv) {
  ...
  float* dSend = nullptr;
  float* dRecv = nullptr;
  CUDACHECK(cudaMalloc(&dSend, count * sizeof(float)));
  CUDACHECK(cudaMalloc(&dRecv, count * sizeof(float)));
  ...
  for (int iter = 0; iter < 3; ++iter) {
    ...
    CUDACHECK(cudaMemcpy(dSend, &input, sizeof(float), cudaMemcpyHostToDevice));
    CUDACHECK(cudaMemset(dRecv, 0, count * sizeof(float)));
    ...
    if (survivor) {
      float observed = 0.0f;
      CUDACHECK(cudaMemcpy(&observed, dRecv, sizeof(float), cudaMemcpyDeviceToHost));
      float expected = 0.0f;
