...
using AllReduceFn = ncclResult_t (*)(const void*, void*, size_t, ncclDataType_t, ncclRedOp_t, ncclComm_t, hipStream_t);
using ReduceScatterFn = ncclResult_t (*)(const void*, void*, size_t, ncclDataType_t, ncclRedOp_t, ncclComm_t, hipStream_t);
using AllGatherFn = ncclResult_t (*)(const void*, void*, size_t, ncclDataType_t, ncclComm_t, hipStream_t);
using BroadcastFn = ncclResult_t (*)(const void*, void*, size_t, ncclDataType_t, int, ncclComm_t, hipStream_t);
using ReduceFn = ncclResult_t (*)(const void*, void*, size_t, ncclDataType_t, ncclRedOp_t, int, ncclComm_t, hipStream_t);
...
void touchFile(const std::string& path, int rank) {
  ...
  char tmp[8192];
  snprintf(tmp, sizeof(tmp), "%s.tmp.%s.%d.%d", path.c_str(), host, rank, getpid());
  FILE* f = fopen(tmp, "wb");
  if (!f) {
...
void writeTextFileIfAbsent(const std::string& path, const std::string& text, int rank) {
  if (exists(path)) return;
  writeTextFile(path, text, rank);
}
...
std::string rankListText(const std::vector<int>& ranks) {
  std::string out;
  for (size_t i = 0; i < ranks.size(); ++i) {
    char buf[32];
    snprintf(buf, sizeof(buf), "%s%d", i == 0 ? "" : ",", ranks[i]);
...
std::vector<int> observedMarkerRanks(const char* prefix) {
  std::vector<int> observed;
  DIR* dir = opendir(bypassDir().c_str());
  if (!dir) return observed;

  std::string head = std::string(prefix) + ".";
  while (struct dirent* ent = readdir(dir)) {
    std::string name(ent->d_name);
    if (name.compare(0, head.size(), head) != 0) continue;
    const char* start = name.c_str() + head.size();
    if (*start == '\0') continue;
    char* end = nullptr;
    errno = 0;
    long rank = strtol(start, &end, 10);
    if (errno == 0 && end != start && *end == '\0') {
      observed.push_back(static_cast<int>(rank));
    }
  }
  closedir(dir);
...
std::vector<int> missingRanksFromObserved(const std::vector<int>& ranks, const std::vector<int>& observed) {
  std::vector<int> missing;
  for (int r : ranks) {
    if (!std::binary_search(observed.begin(), observed.end(), r)) missing.push_back(r);
  }
  return missing;
}

bool waitRanks(const char* prefix, const std::vector<int>& ranks, int timeoutSec,
               int selfRank, const char* phase) {
  auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(timeoutSec);
  std::string releasePath = bypassDir() + "/" + std::string(prefix) + "_release";
  bool supervisorReleaseBarrier = envFlag("FTCCL_SUPERVISOR_RELEASE_BARRIER", false);
  while (std::chrono::steady_clock::now() < deadline) {
    if (supervisorReleaseBarrier) {
      if (exists(releasePath)) return true;
      usleep(10000);
      continue;
    }

    std::vector<int> observed = observedMarkerRanks(prefix);
    if (missingRanksFromObserved(ranks, observed).empty()) return true;

    bool okByStat = true;
    for (int r : ranks) {
      if (!exists(rankPath(prefix, r))) okByStat = false;
    }
    if (okByStat) return true;
    usleep(10000);
  }

  std::vector<int> observed = observedMarkerRanks(prefix);
  std::vector<int> missing = missingRanksFromObserved(ranks, observed);
  if (supervisorReleaseBarrier) {
    logf(0,
         "rank %d timed out waiting for supervisor %s release prefix=%s release=%s timeoutSec=%d sync_dir=%s",
         selfRank, phase, prefix, releasePath.c_str(), timeoutSec, bypassDir().c_str());
  } else {
    logf(0,
         "rank %d timed out waiting for %s barrier prefix=%s timeoutSec=%d missing=[%s] observed=[%s] sync_dir=%s",
         selfRank, phase, prefix, timeoutSec, rankListText(missing).c_str(),
         rankListText(observed).c_str(), bypassDir().c_str());
  }
  return false;
}
...
void publishDetectorBypass(const CommState& st, int failedRank, const char* reason,
  ...
  writeTextFileIfAbsent(pathFor("failed_rank"), std::to_string(failedRank) + "\n", st.parentRank);
  writeTextFileIfAbsent(pathFor("survivors"), joinRanks(survivors), st.parentRank);
  writeTextFileIfAbsent(pathFor("generation"), generation, st.parentRank);
...
bool shouldPropagateRecoverableError(ncclComm_t comm, const CommState& st, const char* where) {
  ...
  writeTextFileIfAbsent(pathFor("failed_rank"), std::to_string(deadRank) + "\n", st.parentRank);
  writeTextFileIfAbsent(pathFor("survivors"), joinRanks(survivors), st.parentRank);
  writeTextFileIfAbsent(pathFor("generation"), configuredGenerationText(), st.parentRank);
...
ncclResult_t performBypassIfNeeded(ncclComm_t comm, const char* where) {
  ...
  logf(0,
       "rank %d starting bypass at %s comm=%p deadRank=%d survivors=%zu timeoutSec=%d sync_dir=%s envTimeout=%s",
       st.parentRank, where, comm, st.deadRank, survivors.size(), timeout, bypassDir().c_str(),
       getenv("FTCCL_BYPASS_TIMEOUT_SEC") ? getenv("FTCCL_BYPASS_TIMEOUT_SEC") : "<unset>");
  gBypassStarted.store(true, std::memory_order_release);
  writeTextFileIfAbsent(pathFor("failed_rank"), std::to_string(st.deadRank) + "\n", st.parentRank);
  writeTextFileIfAbsent(pathFor("survivors"), joinRanks(survivors), st.parentRank);
  writeTextFileIfAbsent(pathFor("generation"), configuredGenerationText(), st.parentRank);
  ...
  if (envFlag("FTCCL_BYPASS_MARKER_BARRIER", false)) {
    if (!waitRanks("prepare", survivors, timeout, st.parentRank, "prepare")) {
      return ncclSystemError;
    }
  }
  ...
  if (envFlag("FTCCL_BYPASS_MARKER_BARRIER", false)) {
    if (!waitRanks("activate", survivors, timeout, st.parentRank, "activate")) {
      return ncclSystemError;
    }
  }
