...
def _wrap_attr_once(module_name: str, attr_name: str, wrapper, marker: str) -> bool:
    module = importlib.import_module(module_name)
    original = getattr(module, attr_name)
    if getattr(original, "_ftccl_patch_marker", None) == marker:
        return False
    replacement = wrapper(original)
    setattr(replacement, "_ftccl_patch_marker", marker)
    setattr(replacement, "_ftccl_original", original)
    setattr(module, attr_name, replacement)
    state.log(f"patched {module_name}.{attr_name}", level=1)
    return True
...
def _install_dataloader_patch() -> None:
    ...
    patched_source = None
    last_error = None
    for module_name in dataloader_modules:
        try:
            wrap_attr(
                module_name,
            ...
            patched_source = module_name
            state.log(f"patched dataloader builder {module_name}", level=1)
            break
    ...
    if patched_source is None:
        raise ModuleNotFoundError(
            f"no supported Megatron data_samplers module found: {last_error}"
        )
    ...
    try:
        ...
        state.log("patched training.py dataloader binding", level=1)
    except Exception:
        pass


def _install_logging_patch() -> None:
    def print_rank_last_wrapper(original):
        @functools.wraps(original)
        def wrapped_print_rank_last(message):
            view = state.current_view()
            if not view.active:
                return original(message)

            mode = state.log_rank_after_bypass()
            should_print = False
            if mode in {"survivor_last", "survivor-last", "last"}:
                should_print = view.survivor_rank == view.survivor_count - 1
            elif mode in {"rank0", "global0", "zero"}:
                should_print = state.global_rank() == 0
            elif mode in {"all", "1", "true"}:
                should_print = view.survivor_rank is not None
            elif mode in {"off", "none", "0", "false"}:
                should_print = False
            else:
                should_print = view.survivor_rank == view.survivor_count - 1
        ...
        return wrapped_print_rank_last

    for module_name in ("megatron.training.utils", "megatron.training.training"):
        try:
            _wrap_attr_once(module_name, "print_rank_last", print_rank_last_wrapper, "ftccl.print_rank_last")
...
def _install_training_patch() -> None:
    ...
    def train_wrapper(original):
        ...
        def wrapped_train(*args, **kwargs):
            train_data_iterator = kwargs.get("train_data_iterator")
            train_iterator_index = 4
            if train_data_iterator is None and len(args) > train_iterator_index:
                train_data_iterator = args[train_iterator_index]
            if train_data_iterator is not None and not isinstance(
                ...
                if "train_data_iterator" in kwargs:
                    kwargs["train_data_iterator"] = train_data_iterator
                elif len(args) > train_iterator_index:
                    args = list(args)
                    args[train_iterator_index] = train_data_iterator
                    args = tuple(args)
        ...
        def wrapped_train_step(*args, **kwargs):
            ...
            def maybe_wrap_optimizer_step():
                ...
                def wrapped_step(*step_args, **step_kwargs):
                    RUNTIME.apply_gradient_compensation(model, optimizer, current_iteration())
            ...
            def current_iteration():
                try:
                    ...
                    return getattr(get_args(), "curr_iteration", iteration)
                except Exception:
                    return iteration
            ...
            while True:
                ...
                try:
                    ...
                    iter_value = current_iteration()
                    if iter_value is not None:
                        state.touch_marker(
                            f"train_step_done.{state.global_rank()}.{iter_value}",
                            f"rank={state.global_rank()} iteration={iter_value}\n",
                        )
                    return result
                except Exception as exc:
                    if (
    ...
    for module_name in ("megatron.training.training", "dcu_megatron.training.training"):
        try:
            _wrap_attr_once(module_name, "train", train_wrapper, "ftccl.train")
        except (ModuleNotFoundError, AttributeError):
            pass
        try:
            _wrap_attr_once(module_name, "train_step", train_step_wrapper, "ftccl.train_step")
        except (ModuleNotFoundError, AttributeError):
            pass
        try:
            _wrap_attr_once(
                ...
                "ftccl.checkpoint_and_decide_exit",
            )
        except (ModuleNotFoundError, AttributeError):
            pass
...
def install() -> None:
    if RUNTIME.installed or not state.enabled():
        return
    state.touch_marker(
        f"pid.{state.global_rank()}",
        (
            f"rank={state.global_rank()} local_rank={state.local_rank()} "
...
def reinstall_training_patch(reason: str = "external-patch") -> None:
    if not state.enabled():
        return
    _install_logging_patch()
    _install_training_patch()
    state.log(f"reinstalled Megatron survivor training patch after {reason}")
