...
namespace RcclUnitTesting
{
  ...
  // Helper function for fp8 e5m2 verification with tolerance
  inline bool verifyncclFp8E5M2(uint8_t const& output, uint8_t const& expected, unsigned const& tolerance)
  {
    uint8_t delta = output < expected ? expected - output : output - expected;
    if(delta > tolerance) 
    {
      return false;
    }
                            ...
                            bool&                 isMatch)
  {
    ...
    for (idx = 0; idx < numElements; ++idx)
    {
      ...
      case ncclFp8E5M2: isMatch = verifyncclFp8E5M2(B1[idx], expected.B1[idx], tolerance); break;
    ...
    if (verbose && !isMatch)
    {
      ...
      case ncclFp8E5M2:
        ERROR("Expected output: %f(%d).  Actual output: %f(%d) at index %lu tolerance %d\n", (float)expected.B1[idx], (int)expected.B1[idx], (float)B1[idx], (int)B1[idx], idx, tolerance); break;
      case ncclBfloat16:
