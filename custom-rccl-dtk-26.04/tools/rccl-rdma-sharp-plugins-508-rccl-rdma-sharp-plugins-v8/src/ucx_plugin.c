...
/* Exclude hip-related UCX transports */
...
static ncclResult_t ucx_config_no_hip(ucp_config_t *config) {
  char tmp[PATH_MAX];
  const char *ucx_tls;
  ssize_t n;
  ...
  if (ucx_tls == NULL) {
    ...
    /* Negative expression, make sure to keep rocm excluded */
    n = snprintf(tmp, sizeof(tmp), "^rocm,%s", &ucx_tls[1]);
    if (n >= sizeof(tmp)) {
      return ncclInternalError;
    }
    ...
    /* Positive expression cannot allow hip-like transports */
    if ((strstr(ucx_tls, "rocm") != NULL) || (strstr(ucx_tls, "gdr") != NULL)) {
      WARN("Cannot use rocm/gdr transports as part of specified UCX_TLS");
      // return ncclInternalError;
    }
  }
...
static ncclResult_t ucx_init_context(ucp_context_h *ctx, int dev) {
  ...
  if (ucp_ctx[dev] == NULL) {
    ...
    if (ncclParamUCXCudaDisable()) {
      result = ucx_config_no_hip(config);
    ...
    // UCXCHECK(ucp_config_modify(config, "RNDV_THRESH", "0"));
    // UCXCHECK(ucp_config_modify(config, "RNDV_SCHEME", "get_zcopy"));
    // UCXCHECK(ucp_config_modify(config, "MEMTYPE_REG_WHOLE_ALLOC_TYPES", "unknown")); 
...
ncclResult_t nccl_ucx_regmr(void* comm, void* data, size_t size, int type, void** mhandle) {
  ...
  mh->mem_type = (type == NCCL_PTR_HOST)? UCS_MEMORY_TYPE_HOST: UCS_MEMORY_TYPE_ROCM;
