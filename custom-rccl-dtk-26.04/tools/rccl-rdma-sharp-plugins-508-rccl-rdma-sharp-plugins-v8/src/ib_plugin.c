...
NCCL_PARAM(IbPciRelaxedOrdering, "IB_PCI_RELAXED_ORDERING", /**2*/0);
NCCL_PARAM(IbFifoTc, "IB_FIFO_TC", 0);
NCCL_PARAM(IbAsyncEvents,"IB_RETURN_ASYNC_EVENTS",1);
NCCL_PARAM(IbEceEnable,"IB_ECE_ENABLE",0); /* 508 CS cannot support ece */
...
typedef struct __BE17{
        __be16 low; ///< 为了兼容竞品SM直接传入be16的数据，低16位放在前面
        __be16 high;
} __be17;

static inline __be17 __be32_to_17(__be32 v){
        __be17 ret;
        u32 val = __builtin_bswap32(v);
        ret.low = __builtin_bswap16((u16)val & 0xffff);
        ret.high = __builtin_bswap16((u16)(val >> 16) & 0xffff);
        return ret;
}
static inline __be32 __be17_to_32(__be17 v){
        u16 high = __builtin_bswap16(v.high);
        u16 low = __builtin_bswap16(v.low);
        u32 val = (high << 16) | low;
...
static inline u17 u32_to_17(u32 v){
        u17 ret;
        ret.low = v & 0xffff;
        ret.high = (v >> 16) & 0xffff;
        return ret;
}
static inline u32 u17_to_32(u17 v){
        return ((v.high << 16) | v.low);
}
static inline u17 be17_to_cpu(__be17 v){
        return u32_to_17(__builtin_bswap32(__be17_to_32(v)));
}
static inline __be17 cpu_to_be17(u17 v){
        return __be32_to_17(__builtin_bswap32(u17_to_32(v)));
}
#define htobe17(x) cpu_to_be17(x)
#define be17toh(x) be17_to_cpu(x)
static inline u17 ntoh17(__be17 v){
        return u32_to_17(__builtin_bswap32(__be17_to_32(v)));
}
static inline __be17 hton17(u17 v){
        return __be32_to_17(__builtin_bswap32(u17_to_32(v)));
}
static inline __be17 pack_be17(__be16 l, u8 h){
        __be17 ret;
        ret.low = l;
        ret.high = (h & 0x01) ? __builtin_bswap16(0x0001) : 0;
        return ret;
}
static inline u17 pack_u17(u16 l, u8 h){
        u17 ret;
        ret.low = l;
        ret.high = (h & 0x01) ? 1 : 0;
        return ret;
}
static inline __be16 unpack_be17_l(__be17 v){
        return v.low;
}
static inline u8 unpack_be17_h(__be17 v){
        u16 high = __builtin_bswap16(v.high);
        return (high & 0x0001) ? 1 : 0;
}
static inline u16 unpack_u17_l(u17 v){
        return v.low;
}
static inline u8 unpack_u17_h(u17 v)
{
        return (v.high & 0x00001) ? 1 : 0;
}
static inline bool is_direct_route_lid(u17 lid) {
        return ((u17_to_32(lid) & 0xffff) == 0xffff);
}

#endif  /* _SHCA_17B_TYPES_H */



// Per-Dev connection metadata
typedef struct ncclIbDevInfo {
  u17 lid;        // modify by zhushy
  uint8_t ib_port;
  enum ibv_mtu mtu;
  uint8_t link_layer;
  uint8_t is_global;
...
#ifndef _SHCA_17B_TYPES_H
    qpAttr.ah_attr.dlid = u17_to_32(info->lid);
#else
...
#ifndef _SHCA_17B_TYPES_H
          qpAttr.ah_attr.dlid = ncclIbExtractFlid(&info->gid/**union ibv_gid gid; */);
#else
          qpAttr.ah_attr.dlid = u32_to_17(ncclIbExtractFlid(&info->gid/**union ibv_gid gid; */));
#endif
...
#ifndef _SHCA_17B_TYPES_H
    devInfo->lid           = u32_to_17(ibDev->portAttr.lid);
#else
...
ib_recv:
  ...
  for (int i = 0; i < mergedDev->ndevs; i++) {
    ...
    if (rComm->flushEnabled) {
      ...
      //devInfo.lid         = ibDev->portAttr.lid;    // no modify by zhushy
    ...
    // Fill Handle
    //meta.devs[i].lid        = ibDev->portAttr.lid;   // no modify by zhushy


#ifndef _SHCA_17B_TYPES_H
              meta.devs[i].lid        = u32_to_17(ibDev->portAttr.lid);
#else
