...
ncclResult_t ncclPrepareTasks(struct ncclComm* comm, bool* algoNeedConnect, bool* needConnect, ncclSimInfo_t* simInfo) {
  ...
  for (int cursor=0; cursor < fnOpTyCount; cursor++) {
    ...
    do {
      ...
      comm->devComm->funcindex = (((agg.func*ncclNumDevRedOps + agg.opDev.op) * ncclNumTypes + agg.datatype) * NCCL_NUM_ALGORITHMS + agg.algorithm) * NCCL_NUM_PROTOCOLS + agg.protocol;
      if (agg.devFuncId < 0) {
...
static ncclResult_t scheduleCollTasksToPlan(
  ...
  int nPlanColls = 0;
  size_t trafficBytes[2*2*2] = {0, 0, 0, 0, 0, 0, 0, 0}; // [collnet][nvls][protocol] 0~3:非SIMPLE（LL或LL128等 4~7:SIMPLE
  int nChannels[2*2*2] = {0, 0, 0, 0, 0, 0, 0, 0}; // [collnet][nvls][protocol]
  int  nMaxChannels[2*2*2] = {comm->nChannels, comm->nvlsChannels, // [collnet][nvls][protocol]
                                 comm->nChannels, comm->nvlsChannels,
                                 comm->CollSimpleChannels, std::min(comm->nvlsChannels,comm->CollSimpleChannels),
                                 comm->CollSimpleChannels, std::min(comm->nvlsChannels,comm->CollSimpleChannels)};
  do {
    size_t workBytes = 0;
    ...
    while (task != nullptr) {
      ...
      int kind = 2*task->isCollnet + task->isNvls + 4*(task->protocol == NCCL_PROTO_SIMPLE ? 1 : 0);
...
static ncclResult_t addP2pToPlan(
    ...
    struct ncclTaskP2p* sendP2p, struct ncclTaskP2p* recvP2p
  ) {
  int connIndex[2] = {1, 1};
  ...
  ssize_t thresholdLL = ncclParamP2pLLThreshold();
  ...
  int userProto[2] = {-1, -1};
  struct ncclTaskP2p* p2pDir[2] = {recvP2p, sendP2p};
  struct ncclTaskP2p* p2p = sendP2p ? sendP2p : recvP2p;
  if (p2p && p2p->UserProto) {
    if (strcasecmp(p2p->UserProto, "SIMPLE") == 0) {
      for (int dir = 0; dir < 2; dir++) userProto[dir] = NCCL_PROTO_SIMPLE;
    } else if (strcasecmp(p2p->UserProto, "LL") == 0 ) {
      for (int dir = 0; dir < 2; dir++) {
        if (protoLL[dir]) userProto[dir] = NCCL_PROTO_LL;
      }
    } else {
      WARN(" The configured value of protocol for p2p : %s is invalid", p2p->UserProto);
    }
  }

  for (int dir=0; dir < 2; dir++) { // 0=recv, 1=send
    if (bytes[dir] != -1) protoLL[dir] &= bytes[dir] <= thresholdLL;
    protocol[dir] = userProto[dir] != -1 ? userProto[dir] : protoLL[dir] ? NCCL_PROTO_LL : NCCL_PROTO_SIMPLE;
    ...
    } else if (comm->nNodes == 1 && p2pDir[dir] && p2pDir[dir]->parentP2pType == P2pAllToAll && protocol[dir] == NCCL_PROTO_SIMPLE && bytes[dir] <= _2MB) {
      chunkSize[dir] = _32KB;
    }
    ...
    if (bytes[dir] == -1) nChannels[dir] = 0;
      ...
      ssize_t maxPartSize = comm->nNodes > 1 ? stepSize[dir]   : comm->minLinkType < PATH_SYS || bytes[dir] < 65536 ? stepSize[dir]*32 : divUp(bytes[dir], comm->p2pnChannels);
    ...
    // UserConfig's nChannels
    if (p2p && p2p->UserChannels) {
      if(p2p->UserChannels <= comm->p2pnChannelsPerPeer){
        if(nChannels[dir]) nChannels[dir] = p2p->UserChannels;
      } else { 
        WARN("UserChannels:%d must not be greater than comm->p2pnChannelsPerPeer:%d ! ", p2p->UserChannels, comm->p2pnChannelsPerPeer);
      }
    }
  }
  ...
  INFO(NCCL_TASK,"p2p : rank %d send rank %d recv rank %d p2pnChannelsPerPeer %d p2pnChannels %d nChannelsMax %d  UserChannels %d protocol %s", 
    comm->rank, sendRank, recvRank, comm->p2pnChannelsPerPeer, comm->p2pnChannels, nChannelsMax, p2p->UserChannels,ncclProtoStr[protocol[0]]);
    ...
    for (int dir=0; dir < nProxyOps; dir++) {
      ...
      if (nParts <= part) {
        ...
        if (proxyOps[dir].reg) {
          ...
          size_t nbytes = partEnd-partBeg;
          proxyOps[dir].nsteps = nbytes ? divUp(nbytes, chunkDataSize) : 1;
...
static ncclResult_t scheduleP2pTasksToPlan(
  ...
  while (comm->planner.nTasksP2p != 0) {
    for (int i=0; i < nRanks; i++) {
      int round = (i+comm->p2pPreRound)%nRanks;
...
static void waitWorkFifoAvailable(struct ncclComm* comm, uint32_t desiredProduced) {
  ...
  while (true) {
    ...
    consumedLeast = comm->preWorkFifoConsumed;
...
ncclResult_t ncclLaunchFinish(struct ncclComm* comm) {
  ...
  if (!ncclIntruQueueEmpty(&planner->planQueue)) {
    ...
    if (comm->workFifoProduced - comm->preWorkFifoConsumed > comm->workFifoBytes/8) {
      comm->preWorkFifoConsumed = comm->workFifoProduced;
    }
...
static ncclResult_t topoGetAlgoInfo(
  ...
  // Configure Coll's protocol , algorithm 
  if(info->UserProto) {
    if (strcasecmp(info->UserProto, "SIMPLE") == 0) {
      info->protocol = NCCL_PROTO_SIMPLE;
    }else if (strcasecmp(info->UserProto, "LL") == 0) {
      info->protocol = NCCL_PROTO_LL;
    }else{
      WARN("The configured value of protocol for coll :%s is invalid",info->UserProto);
    }
  }
  if(info->UserAlgo) {
    if (strcasecmp(info->UserAlgo, "RING") == 0) {
      info->algorithm = NCCL_ALGO_RING;
    }else if (strcasecmp(info->UserAlgo, "TREE") == 0 && info->func == ncclFuncAllReduce) {
      info->algorithm = NCCL_ALGO_TREE;
    }else {
      WARN("The configured value of algorithm for coll : %s is invalid",info->UserAlgo);
    }
  }

  int nc = comm->nChannels;
  if (info->protocol == NCCL_PROTO_SIMPLE) {
    nc = comm->CollSimpleChannels;
    // 针对reduce和broadcast特性定制tuning
    if (IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx936") && comm->nNodes > 1 && info->func <= ncclFuncReduce && info->count*ncclTypeSize(info->datatype) < 1*1024*1024){
      nc = 1;
    }
  }
  ...
  // Configure the actual nChannels through UserChannels
  if (info->UserChannels > 0 && info->UserChannels <= comm->nChannels) {
    int MaxRealNc = DIVUP(nBytes,nt);
    int UserNc = std::max(std::min(2*CHANNEL_LIMIT, MaxRealNc),1);
    if (info->UserChannels > UserNc) {
      info->nMaxChannels = UserNc;
      WARN("The configured value of Coll nChannels : %d is unreasonable, Coll nChannels is adjusted to %d ",info->UserChannels,UserNc);
    } else if(info->UserChannels >= nc) {
      info->nMaxChannels = info->UserChannels;
    } else {
      info->nMaxChannels = info->UserChannels;
      WARN("The configured value of Coll nChannels : %d is unreasonable which may cause performance degradation, it should be >=  %d ",info->UserChannels,nc);
    }
  } else if(info->UserChannels) {
    WARN("The configured value of Coll nChannels : %d is invalid",info->UserChannels);
  }
...
static ncclResult_t calcCollChunking(
  ...
  if (info->algorithm == NCCL_ALGO_TREE && info->protocol == NCCL_PROTO_SIMPLE) {
    ...
    if (pattern == ncclPatternPipelineFrom || pattern == ncclPatternPipelineTo) {
      ...
      if (!IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx906") &&
          !IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx926") &&
          !IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx928")) {
  ...
  *outChunkSize = chunkSize;
  ...
  INFO(NCCL_TASK,"Global rank %d : opCount %lx slicesteps %d spl %d cpl %d nbytes %zi -> protocol %s algorithm %s nchannels %d , nloops %d nsteps %d chunksize %d comm %p ",comm->localRankToRank[comm->localRank],proxyOp->opCount, sliceSteps, nstepsPerLoop, nchunksPerLoop, nBytes, 
    ncclProtoStr[info->protocol], ncclAlgoStr[info->algorithm], info->nMaxChannels, nLoops, proxyOp->nsteps, chunkSize, comm);
  return ncclSuccess;
}
...
static const int PerPeerChannelsConfigGfx936[][6] = {
  // Index: [SingleNodeAlltoAll, SingleNodeGather, SingleNodeScatter, MultiNodesAlltoAll, MultiNodesGather, MultiNodesScatter]
  {2,2,2,1,2,1},    /* 0: 1B-8KB */
  {2,4,4,1,2,1},    /* 1: 16KB   */
  {2,4,4,1,2,1},    /* 2: 32KB   */
  {2,4,4,1,2,1},    /* 3: 64KB   */
  {4,4,2,1,2,1},    /* 4: 128KB  */
  ...
  {4,4,4,1,1,1},    /* 8: 2MB    */
  {4,2,16,1,2,1},   /* 9: 4MB    */
  {16,2,16,1,2,1},  /* 10: 8MB   */
  {16,2,16,1,2,1},  /* 11: 16MB  */
  {16,4,16,1,2,1},  /* 12: 32MB  */
  {4,4,4,1,2,1},    /* 13: 64MB  */
  {16,4,16,1,2,1},  /* 14: 128MB */
  {16,4,16,1,2,1},  /* 15: 256MB */
  {16,4,16,1,2,1},  /* 16: 512MB */
  {16,4,16,1,2,1},  /* 17: 1GB   */
  {16,16,16,1,2,1}  /* 18: 2GB   */
};

static const int PerPeerChannelsConfigGfx938[][6] = {
  // Index: [SingleNodeAlltoAll, SingleNodeGather, SingleNodeScatter, MultiNodesAlltoAll, MultiNodesGather, MultiNodesScatter]
  {2,2,2,1,2,1},    /* 0: 1B-8KB */
  {4,4,2,1,2,1},    /* 1: 16KB   */
  {4,4,2,1,2,1},    /* 2: 32KB   */
  ...
  {2,4,4,1,2,1},    /* 13: 64MB  */
  {16,4,16,1,2,1},  /* 14: 128MB */
  {16,16,16,1,2,1}, /* 15: 256MB */
  {16,16,16,1,2,1}, /* 16: 512MB */
  {16,16,16,1,2,1}, /* 17: 1GB   */
  {16,16,16,1,2,1}  /* 18: 2GB   */
};

static int getP2pPerPeerChannels(struct ncclComm* comm, struct ncclInfo* info, ncclP2pInfo_t parentP2p) {
  ssize_t totalSize = info->count * ncclTypeSize(info->datatype);
  int index = log2i(totalSize >> 13);
  index = index < 0 ? 0 : (index >= 18 ? 18 : index);
  int arrayIndex;
  if (comm->nNodes == 1) {
    switch (parentP2p) {
      case P2pAllToAll: arrayIndex = 0; break;
      case P2pGather:   arrayIndex = 1; break;
      case P2pScatter:  arrayIndex = 2; break;
      default: return comm->oriP2pnChannelsPerPeer;
    }
  } else {
    switch (parentP2p) {
      case P2pAllToAll: arrayIndex = 3; break;
      case P2pGather:   arrayIndex = 4; break;
      case P2pScatter:  arrayIndex = 5; break;
      default: return comm->oriP2pnChannelsPerPeer;
    }
  }
  return (IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx936") ? PerPeerChannelsConfigGfx936 : PerPeerChannelsConfigGfx938)[index][arrayIndex];
}
...
static ncclResult_t taskAppend(struct ncclComm* comm, struct ncclInfo* info) {
  struct ncclKernelPlanner *planner = &comm->planner;
  comm->devComm->nBytes = info->count*ncclTypeSize(info->datatype);
  ...
  if (info->coll == ncclFuncSend || info->coll == ncclFuncRecv) {
    ...
    p2p->UserProto = info->UserProto;
    p2p->UserChannels = info->UserChannels;
    p2p->parentP2pType = P2pInvalid;
    ncclIntruQueueEnqueue(
      isSendNotRecv ? &planner->peers[peer].sendQueue : &planner->peers[peer].recvQueue,
      p2p);
    planner->nTasksP2p += 1;
    if (comm->autoPerPeerOn) {
      p2p->parentP2pType = currentParentP2p();
      if (planner->nTasksP2p == 1) {
        auto updateChannels = [&](int channels) {
          comm->p2pnChannelsPerPeer = std::min(channels, comm->p2pnChannels);
          comm->devComm->p2pnChannelsPerPeer = comm->p2pnChannelsPerPeer;
        };
        auto maybeRestore = [&]() {
          if (comm->p2pnChannelsPerPeer != comm->oriP2pnChannelsPerPeer) {
            updateChannels(comm->oriP2pnChannelsPerPeer);
          }
        };
        switch (p2p->parentP2pType) {
          case P2pAllToAll:
          case P2pGather:
          case P2pScatter:
            updateChannels(getP2pPerPeerChannels(comm, info, p2p->parentP2pType));
            break; 
          case P2pAllToAllv:
            comm->nNodes == 1 ? updateChannels(4) : maybeRestore();
            break;
          case P2pInvalid:
            comm->nNodes > 1 ? updateChannels(16) : maybeRestore();
            break;
          default:
            maybeRestore();
            break;
        }
        if (IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx938") && p2p->bytes >= _128MB && comm->nNodes == 1 && p2p->parentP2pType == P2pAllToAll && comm->oriP2pnChannels > 16) {
          comm->p2pnChannels = 16;
        } else if (comm->p2pnChannels != comm->oriP2pnChannels){
          comm->p2pnChannels = comm->oriP2pnChannels;
        }
      }
    }
    // Mark channels that need pre-connect
    if (comm->rank != peer) {
      uint64_t& seenMask = isSendNotRecv ? planner->peers[peer].sendSeenMask : planner->peers[peer].recvSeenMask;
      uint64_t bit = 1ULL << (comm->p2pnChannelsPerPeer - 1);
      if ((seenMask & bit) == 0) {
        seenMask |= bit;
        int round = 0;
    ...
    if (comm->nRanks == 1) {
      ...
      t->UserProto = info->UserProto;
      t->UserAlgo = info->UserAlgo;
      t->UserChannels = info->UserChannels;
