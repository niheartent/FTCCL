...
typedef enum {
  HCU_LINK_MODE_NON = 0,
  HCU_LINK_MODE_XHCL_DIRECT = 1,    // DCU XHCL直连
  HCU_LINK_MODE_XHCL_HYSWITCH = 2,  // DCU 连接hyswitch
  HCU_LINK_MODE_ONLY_PCIE = 3,      // DCU 只有PCIe互联，无XHCL
} HCU_LINK_MODE_TYPE;
...
struct ncclTaskColl {
  ...
  void* recvMhandle;
  uint64_t opCount;
  //Optional User Configuration Parameter: Protocol Sting/Algorithm String/nChannels
  const char* UserProto;
  const char* UserAlgo;
  int UserChannels;
};
struct ncclTaskP2p {
...
struct ncclKernelPlanner {
  ...
  struct Peer {
    uint64_t sendSeenMask, recvSeenMask;
...
struct ncclComm {
  ...
  int CollSimpleChannels; //Collectives Simple nChannels
  ...
  int oriP2pnChannelsPerPeer;
  bool autoPerPeerOn;
  int minLinkType; // 所有gpu间的最快的链接类型
  ...
  int ccldEnabled;
  struct ncclCcldChannelMetrics* ccldMetrics;
