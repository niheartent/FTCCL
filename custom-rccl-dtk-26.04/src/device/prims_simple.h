...
private:
  ...
  __device__ __forceinline__ void ccldAtomicAdd(uint64_t* ptr, uint64_t value) {
    atomicAdd(reinterpret_cast<unsigned long long*>(ptr),
              static_cast<unsigned long long>(value));
  }

  __device__ __forceinline__ void ccldAtomicMax(uint64_t* ptr, uint64_t value) {
    atomicMax(reinterpret_cast<unsigned long long*>(ptr),
              static_cast<unsigned long long>(value));
  }

  __device__ __forceinline__ void ccldStore(uint64_t* ptr, uint64_t value) {
    __atomic_store_n(ptr, value, __ATOMIC_RELAXED);
  }

  __device__ __forceinline__ void ccldRecordRecv(int nelts, uint64_t waitSpins) {
    if (!ncclShmem.comm.ccldEnabled || ncclShmem.comm.ccldMetrics == nullptr) return;
    struct ncclCcldChannelMetrics* metrics = ncclShmem.comm.ccldMetrics + ncclShmem.channelId;
    uint64_t bytes = static_cast<uint64_t>(nelts > 0 ? nelts : 0) * sizeof(T);
    uint64_t ts = wall_clock64();
    ccldAtomicAdd(&metrics->recvCount, 1);
    ccldAtomicAdd(&metrics->recvBytes, bytes);
    ccldAtomicAdd(&metrics->recvWaitSpins, waitSpins);
    ccldAtomicMax(&metrics->recvMaxWaitSpins, waitSpins);
    ccldStore(&metrics->lastRecvTimestamp, ts);
    ccldStore(&metrics->lastAnyTimestamp, ts);
  }

  __device__ __forceinline__ void ccldRecordSend(int nelts) {
    if (!ncclShmem.comm.ccldEnabled || ncclShmem.comm.ccldMetrics == nullptr) return;
    struct ncclCcldChannelMetrics* metrics = ncclShmem.comm.ccldMetrics + ncclShmem.channelId;
    uint64_t bytes = static_cast<uint64_t>(nelts > 0 ? nelts : 0) * sizeof(T);
    uint64_t ts = wall_clock64();
    ccldAtomicAdd(&metrics->sendCount, 1);
    ccldAtomicAdd(&metrics->sendBytes, bytes);
    ccldAtomicAdd(&metrics->sendPostCount, 1);
    ccldStore(&metrics->lastSendTimestamp, ts);
    ccldStore(&metrics->lastAnyTimestamp, ts);
  }

  __device__ __forceinline__ void ccldRecordRecvPost() {
    if (!ncclShmem.comm.ccldEnabled || ncclShmem.comm.ccldMetrics == nullptr) return;
    struct ncclCcldChannelMetrics* metrics = ncclShmem.comm.ccldMetrics + ncclShmem.channelId;
    uint64_t ts = wall_clock64();
    ccldAtomicAdd(&metrics->recvPostCount, 1);
    ccldStore(&metrics->lastAnyTimestamp, ts);
  }
  ...
  __device__ __forceinline__ void waitPeer(intptr_t srcIx, intptr_t dstIx, int offset, int nelts) {
    ...
    if (flags & (Recv*RoleWaitRecv | Send*RoleWaitSend)) {
      ...
      if (!isSendNotRecv && (flags & (Recv * RoleWaitRecv))) ccldRecordRecv(nelts, waitSpins);
      if (isSendNotRecv && (flags & (Send * RoleWaitSend)) && ncclShmem.comm.ccldEnabled && ncclShmem.comm.ccldMetrics != nullptr) {
        struct ncclCcldChannelMetrics* metrics = ncclShmem.comm.ccldMetrics + ncclShmem.channelId;
        ccldAtomicAdd(&metrics->sendWaitSpins, waitSpins);
        ccldAtomicMax(&metrics->sendMaxWaitSpins, waitSpins);
      }
      step += StepPerSlice;
    }
  }
  ...
  inline __device__ void postPeer(bool dataStored, int nelts = 0) {
    ...
    if (flags & (Recv*RolePostRecv | Send*RolePostSend)) {
      step += StepPerSlice;
      if (Send && (flags & RolePostSend) && (dataStored||(flags&ConnFifoEnabled))) ccldRecordSend(nelts);
      if (Recv && (flags & RolePostRecv)) ccldRecordRecvPost();
  ...
  __device__ __forceinline__ void genericOp(
    ...
    if (tid < nworkers && offset < nelem && ((flags & UserBufferMode) == 0)) {
      ...
      do {
        ...
        postPeer<Recv, Send>(0 < sliceSize, sliceSize);
        offset += sliceSize;
        slice += 1;
    ...
    for (int slice=0; slice < SlicePerChunk; slice++) {
      if (tid < nworkers) {
        if (flags & (Recv*RoleWaitRecv | Send*RoleWaitSend)) {
          ...
          if (!isSendNotRecv && (flags & RoleWaitRecv)) ccldRecordRecv(stepSize * StepPerSlice, waitSpins);
          if (isSendNotRecv && (flags & RoleWaitSend) && ncclShmem.comm.ccldEnabled && ncclShmem.comm.ccldMetrics != nullptr) {
            struct ncclCcldChannelMetrics* metrics = ncclShmem.comm.ccldMetrics + ncclShmem.channelId;
            ccldAtomicAdd(&metrics->sendWaitSpins, waitSpins);
            ccldAtomicMax(&metrics->sendMaxWaitSpins, waitSpins);
          }
...
private:
  ...
  ScatterGatherOp(intptr_t inpIx, intptr_t outIx, ssize_t totalElem, int peerElem, ssize_t peerOffset, int skip, int shift, bool postOp) {
    ...
    for (int slice=0; slice<SlicePerChunk; ++slice) {
      ...
      postPeer<Recv, Send>(fenceNeeded, realSize);
      offset += realSize;
    }
  }
