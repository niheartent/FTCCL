...
RCCL_PARAM(DebugSleep, "DEBUG_SLEEP", 0);
void debug_sleep()
{
    int sleepSecs = rcclParamDebugSleep();
    if (sleepSecs != 0)
    {
      char hostname[256] = {0};
...
ncclResult_t commSetUnrollFactor(struct ncclComm* comm) {
  ...
  if((IsArchMatch(devProp.gcnArchName, "gfx94") && devProp.multiProcessorCount > 80))
...
static ncclResult_t devCommSetup(ncclComm_t comm) {
  ...
  comm->preWorkFifoConsumed = 0;
  comm->workFifoConsumedLeast = 0;
  tmpCommAndChans.comm.workConsumed = comm->workFifoConsumed;

  comm->ccldEnabled = ncclParamCcldEnable() != 0;
  tmpCommAndChans.comm.ccldEnabled = comm->ccldEnabled;
  if (comm->ccldEnabled) {
    NCCLCHECKGOTO(ncclCudaHostCalloc(&comm->ccldMetrics, MAXCHANNELS), ret, fail);
    tmpCommAndChans.comm.ccldMetrics = comm->ccldMetrics;
    ncclCommPushCudaHostFree(comm, comm->ccldMetrics);
    INFO(NCCL_INIT, "CCL-D kernel primitive metrics enabled");
  }
...
#define DEFAULT_BUFFSIZE (1 << 21) /* 2MiB */
...
static ncclResult_t computeBuffSizes(struct ncclComm* comm) {
  ...
  if (IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx938") && comm->nNodes > 1) comm->buffSizes[NCCL_PROTO_SIMPLE] = (1 << 20) ;
...
RCCL_PARAM(NetGdrWriteUseXdpEnable, "GDR_WRITE_USE_XDP_ENABLE", 1);

/* 获取本地rank的hsa agent */
ncclResult_t getLocalHsaAgent(struct ncclComm* comm)
{
  struct hsaAgentInfo agentInfo;

  if (comm->validHsaAgent) {
    return ncclSuccess;
  }

  agentInfo.rank = comm->rank;
  agentInfo.busId = comm->busId;
  NCCLCHECK(rocmGetHsaAgentFromBdf(&agentInfo));
  if (agentInfo.validHsaAgent) {
    comm->validHsaAgent = true;
    comm->hsaAgent = agentInfo.hsaAgent;
    return ncclSuccess;
  } else {
    comm->validHsaAgent = false;
    return ncclInternalError;
  }
}

/* 获取本节点DCU的互联方式 */
ncclResult_t getHwLinkTopo(struct ncclComm* comm)
{
  uint32_t deviceCnt = 0;
  uint32_t linkNum = 0;
  uint32_t linkState = 0;
  uint64_t pciBdf = 0;
  RSMI_XHCL_LINK_TYPE devType;

  comm->linkTopoType = HCU_LINK_MODE_NON;
#ifdef RSMI_SUPPORT_XHCL_TOPO
  NCCLCHECK(rocm_smi_getNumDevice(&deviceCnt));
  for (int dev = 0; dev < deviceCnt; dev++) {
    NCCLCHECK(rocm_smi_dev_xhcl_link_number_get(dev, &linkNum));
    if (linkNum == 0) {
      comm->linkTopoType = HCU_LINK_MODE_ONLY_PCIE;
      INFO(NCCL_INIT, "Rank:%d link topo type is only pcie",  comm->rank);
      return ncclSuccess;
    }

    for (int link = 0; link < linkNum; link++) {
      NCCLCHECK(rocm_smi_dev_xhcl_link_state_get(dev, link, &linkState));
      if (linkState == 0) {
        continue;
      }

      NCCLCHECK(rocm_smi_dev_xhcl_link_remote_dev_type_get(dev, link, &devType));
      NCCLCHECK(rocm_smi_dev_xhcl_link_remote_bdfid_get(dev, link, &pciBdf));
      if (devType == RSMI_XHCL_LINK_TYPE_SWITCH) {
        comm->linkTopoType = HCU_LINK_MODE_XHCL_HYSWITCH;
      } else if (devType == RSMI_XHCL_LINK_TYPE_GPU) {
        comm->linkTopoType = HCU_LINK_MODE_XHCL_DIRECT;
      } else {
        continue;
      }

      INFO(NCCL_INIT, "Rank:%d Get link topo from rocmDev:%d linkNum:%d link:%d linkState:%d remote type:%d bdf:0x%lx topoType:%d", 
        comm->rank, dev, linkNum, link, linkState, devType, pciBdf, comm->linkTopoType);
      return ncclSuccess;
    }
  }

  WARN("Rank:%d fail to get link topo type deviceCnt %d", comm->rank, deviceCnt);
  return ncclInternalError;
#else
  return ncclSuccess;
#endif
}
/* 读系统驱动文件xdp_size，判断是否开启XDP */
bool checkXdpStatus() {
  FILE *file = NULL;
  int xdp_value = 0;
  int result = 0;

  file = fopen("/sys/module/hycu/parameters/xdp_size", "r");
  if (file == NULL) {
    INFO(NCCL_INIT, "Could not open /sys/module/hycu/parameters/xdp_size, XDP is disabled");
    return false;  // 文件不存在或无法打开
  }
  // 读取文件中的值
  if (fscanf(file, "%d", &xdp_value) == 1) {
    if(xdp_value != 0) 
      INFO(NCCL_INIT, "XDP Switch is on with xdp_size : %d", xdp_value);
    else 
      INFO(NCCL_INIT, "XDP Switch is off with xdp_size : %d", xdp_value);
    result = (xdp_value != 0);
  }
  fclose(file);
  return result;
...
NCCL_PARAM(SharedNetEnable, "SHARED_NET_ENABLE", 0);
NCCL_PARAM(CollSimpleChannels, "SIMPLE_CHANNELS", -2);
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  getLocalHsaAgent(comm);
  comm->useXdp = rcclParamNetGdrWriteUseXdpEnable() && checkXdpStatus();
  ...
  NCCLCHECK(getHwLinkTopo(comm));
  INFO(NCCL_INIT, "Transport init rank %d useXdp %d linkTopoType %d", comm->rank, comm->useXdp, comm->linkTopoType);

  // User-configured Graph
  comm->topo->UserGraphFile = comm->config.graph;
  ...
  if (IsArchMatch(comm->topo->nodes[GPU].nodes[idx].gpu.gcn, "gfx928") || IsArchMatch(comm->topo->nodes[GPU].nodes[idx].gpu.gcn, "gfx906"))
    allGather3Data[rank].nc = 4;
  if (IsArchMatch(comm->topo->nodes[GPU].nodes[idx].gpu.gcn, "gfx93"))
    allGather3Data[rank].nc = CHANNEL_LIMIT*2;
  ...
  comm->CollSimpleChannels = comm->nChannels;
  // set Simple nChannels to ncclParamCollSimpleChannels() when gcn=gfx936 or multi nodes on gfx938
  if(IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx936") || 
    ((IsArchMatch(comm->topo->nodes[GPU].nodes[0].gpu.gcn, "gfx938")) && comm->nNodes > 1)) {
    int CollSimpleChannels = 16;
    if(ncclParamCollSimpleChannels() != -2) CollSimpleChannels = ncclParamCollSimpleChannels();
    if(CollSimpleChannels <= comm->nChannels && CollSimpleChannels > 0) comm->CollSimpleChannels = CollSimpleChannels;
  }
...
static ncclResult_t parseCommConfig(ncclComm_t comm, ncclConfig_t *config) {
  ...
  NCCL_CONFIG_DEFAULT(internalConfigPtr, graph, NCCL_CONFIG_UNDEF_PTR, NULL, "Graph", "%s");
  ...
  comm->config.graph = internalConfigPtr->graph;
...
NCCL_API(ncclResult_t, ncclCommPrepareSurvivorTopo, ncclComm_t comm, int survivorCount, const int* survivorRanks);
NCCL_API(ncclResult_t, ncclCommActivateSurvivorTopo, ncclComm_t comm, int survivorCount, const int* survivorRanks);

static ncclResult_t ncclCommValidateSurvivors(ncclComm_t comm, int survivorCount,
    const int* survivorRanks, int* survivorIndexOut) {
  ncclResult_t ret = ncclSuccess;
  int* seen = NULL;
  int oldNRanks;
  int survivorIndex = -1;
  ...
  NCCLCHECK(PtrCheck((void*)survivorRanks, __func__, "survivorRanks"));

  oldNRanks = comm->nRanks;
  if (survivorCount <= 0 || survivorCount > oldNRanks) {
    WARN("%s: invalid survivorCount %d for comm nranks %d", __func__, survivorCount, oldNRanks);
    return ncclInvalidArgument;
  }

  NCCLCHECKGOTO(ncclCalloc(&seen, oldNRanks), ret, fail);
  for (int i = 0; i < survivorCount; i++) {
    int rank = survivorRanks[i];
    if (rank < 0 || rank >= oldNRanks) {
      WARN("%s: survivorRanks[%d]=%d is outside 0..%d", __func__, i, rank, oldNRanks-1);
      ret = ncclInvalidArgument;
      goto fail;
    }
    if (seen[rank]) {
      WARN("%s: duplicate survivor rank %d", __func__, rank);
      ret = ncclInvalidArgument;
      goto fail;
    }
    seen[rank] = 1;
    if (rank == comm->rank) survivorIndex = i;
  }

  *survivorIndexOut = survivorIndex;
...
static void ncclCommClearSurvivorConnectMasks(struct ncclComm* comm, int connIndex) {
  int offset = CHANNEL_MASK_OFFSET(comm->nRanks, connIndex);
  int numMasks = MAXCHANNELS/64;
  for (int peer = 0; peer < comm->nRanks; peer++) {
    for (int i = 0; i < numMasks; i++) {
      comm->connectSend[peer + offset].masks[i] = 0UL;
      comm->connectRecv[peer + offset].masks[i] = 0UL;
    }
  }
}
...
ncclResult_t ncclCommPrepareSurvivorTopo(ncclComm_t comm, int survivorCount, const int* survivorRanks) {
  ...
  NCCLCHECKGOTO(ncclCommValidateSurvivors(comm, survivorCount, survivorRanks, &survivorIndex),
                ret, fail);

  ncclCommClearSurvivorConnectMasks(comm, 0);

  if (survivorIndex != -1 && survivorCount > 1) {
    int prev = survivorRanks[(survivorIndex + survivorCount - 1) % survivorCount];
    int next = survivorRanks[(survivorIndex + 1) % survivorCount];
    for (int c = 0; c < comm->nChannels; c++) {
      NCCLCHECKGOTO(ncclTransportP2pConnect(comm, c, 1, &prev, 1, &next, 0), ret, fail);
    }
  }
  ...
  if (survivorIndex != -1) {
    INFO(NCCL_INIT, "%s comm %p parentRank %d survivorRank %d survivorCount %d prepared",
         __func__, comm, comm->rank, survivorIndex, survivorCount);
  } else {
    INFO(NCCL_INIT, "%s comm %p parentRank %d excluded from survivor topology view",
         __func__, comm, comm->rank);
  }
...
static ncclResult_t ncclCommSetSurvivorTopoUserRanks(struct ncclComm* comm,
    int survivorCount, int survivorIndex) {
  for (int c = 0; c < comm->nChannels; c++) {
    struct ncclRing* ring = &comm->channels[c].ring;
    for (int i = 0; i < survivorCount; i++) {
      ring->userRanks[i] = (survivorIndex + i) % survivorCount;
    }
  }
  return ncclSuccess;
}
...
ncclResult_t ncclCommActivateSurvivorTopo(ncclComm_t comm, int survivorCount, const int* survivorRanks) {
  ...
  if (survivorIndex != -1) {
    int prev = survivorCount == 1 ? comm->rank :
      survivorRanks[(survivorIndex + survivorCount - 1) % survivorCount];
    int next = survivorCount == 1 ? comm->rank :
      survivorRanks[(survivorIndex + 1) % survivorCount];
    struct ncclDevChannel* devChannels =
      (struct ncclDevChannel*)((char*)comm->devComm + offsetof(struct ncclDevCommAndChannels, channels));

    comm->nRanks = survivorCount;
    for (int c = 0; c < comm->nChannels; c++) {
      comm->channels[c].ring.prev = prev;
      comm->channels[c].ring.next = next;
      comm->channels[c].ring.index = survivorIndex;
    }
    NCCLCHECKGOTO(ncclCommSetSurvivorTopoUserRanks(comm, survivorCount, survivorIndex),
                  ret, fail);

    NCCLCHECKGOTO(ncclStrongStreamAcquireUncaptured(&comm->sharedRes->deviceStream), ret, fail);
    NCCLCHECKGOTO(ncclCudaMemcpyAsync(&comm->devComm->nRanks, &survivorCount, 1,
                                      comm->sharedRes->deviceStream.cudaStream), ret, release_stream);
    NCCLCHECKGOTO(ncclCudaMemcpyAsync(&comm->devComm->rank, &survivorIndex, 1,
                                      comm->sharedRes->deviceStream.cudaStream), ret, release_stream);
    for (int c = 0; c < comm->nChannels; c++) {
      struct ncclRing ring = comm->channels[c].ring;
      NCCLCHECKGOTO(ncclCudaMemcpyAsync(comm->channels[c].devRingUserRanks, ring.userRanks,
                                        survivorCount, comm->sharedRes->deviceStream.cudaStream), ret, release_stream);
      ring.userRanks = comm->channels[c].devRingUserRanks;
      NCCLCHECKGOTO(ncclCudaMemcpyAsync(&devChannels[c].ring, &ring, 1,
                                        comm->sharedRes->deviceStream.cudaStream), ret, release_stream);
    }
release_stream:
    { ncclResult_t syncRet = ncclStrongStreamSynchronize(&comm->sharedRes->deviceStream);
      ncclResult_t releaseRet =
        ncclStrongStreamRelease(ncclCudaGraphNone(), &comm->sharedRes->deviceStream);
      if (ret == ncclSuccess) ret = syncRet;
      if (ret == ncclSuccess) ret = releaseRet;
      NCCLCHECKGOTO(ret, ret, fail);
    }
    INFO(NCCL_INIT, "%s comm %p parentRank %d survivorRank %d survivorCount %d prev %d next %d",
         __func__, comm, comm->rank, survivorIndex, survivorCount, prev, next);
  } else {
    INFO(NCCL_INIT, "%s comm %p parentRank %d excluded from survivor topology view",
         __func__, comm, comm->rank);
  }
...
NCCL_API(ncclResult_t, ncclCommCcldGetMetrics, const ncclComm_t comm, int maxChannels, void* outMetrics, size_t outBytes);
ncclResult_t ncclCommCcldGetMetrics(const ncclComm_t comm, int maxChannels, void* outMetrics, size_t outBytes) {
  NVTX3_FUNC_RANGE_IN(nccl_domain);

  NCCLCHECK(CommCheck(comm, "CommCcldGetMetrics", "comm"));
  NCCLCHECK(PtrCheck(outMetrics, "CommCcldGetMetrics", "outMetrics"));
  NCCLCHECK(ncclCommEnsureReady(comm));

  if (maxChannels < 0) return ncclInvalidArgument;
  int nChannels = std::min(maxChannels, MAXCHANNELS);
  size_t required = static_cast<size_t>(nChannels) * sizeof(struct ncclCcldChannelMetrics);
  if (outBytes < required) return ncclInvalidArgument;
  if (!comm->ccldEnabled || comm->ccldMetrics == nullptr) {
    memset(outMetrics, 0, required);
    return ncclSuccess;
  }
  memcpy(outMetrics, comm->ccldMetrics, required);
  return ncclSuccess;
}

NCCL_API(ncclResult_t, ncclCommCcldResetMetrics, ncclComm_t comm);
ncclResult_t ncclCommCcldResetMetrics(ncclComm_t comm) {
  NVTX3_FUNC_RANGE_IN(nccl_domain);

  NCCLCHECK(CommCheck(comm, "CommCcldResetMetrics", "comm"));
  NCCLCHECK(ncclCommEnsureReady(comm));
  if (comm->ccldEnabled && comm->ccldMetrics != nullptr) {
    memset(comm->ccldMetrics, 0, MAXCHANNELS * sizeof(struct ncclCcldChannelMetrics));
  }
  return ncclSuccess;
}
