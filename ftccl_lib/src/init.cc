...
static ncclResult_t ncclCommValidateSurvivors(ncclComm_t comm, int survivorCount,
  ...
  NCCLCHECK(PtrCheck(survivorRanks, __func__, "survivorRanks"));
...
ncclResult_t ncclCommPrepareSurvivorTopo(ncclComm_t comm, int survivorCount, const int* survivorRanks) {
  ...
  // Keep comm->nRanks at the parent size while setting up transport. P2P setup
  // only exchanges bootstrap data with peers whose connect masks are set, so
  // survivor ranks can run this after a planned failed rank has exited.
...
static ncclResult_t ncclCommSetSurvivorTopoUserRanks(struct ncclComm* comm, int parentNRanks,
  ...
  for (int c = 0; c < comm->nChannels; c++) {
    ...
    if (ring->rankToIndex != NULL) {
      for (int i = 0; i < parentNRanks; i++) ring->rankToIndex[i] = -1;
      for (int i = 0; i < survivorCount; i++) ring->rankToIndex[ring->userRanks[i]] = i;
    }
  }
  return ncclSuccess;
}
...
ncclResult_t ncclCommActivateSurvivorTopo(ncclComm_t comm, int survivorCount, const int* survivorRanks) {
  ...
  if (survivorIndex != -1) {
    ...
    int parentNRanks = comm->nRanks;
    cudaStream_t deviceStream;
    struct ncclDevChannel* devChannels =
      (struct ncclDevChannel*)((char*)comm->devComm + offsetof(struct ncclKernelCommAndChannels, channels));
    ...
    NCCLCHECKGOTO(ncclCommSetSurvivorTopoUserRanks(comm, parentNRanks, survivorCount, survivorIndex),
                  ret, fail);

    NCCLCHECKGOTO(ncclStrongStreamAcquire(ncclCudaGraphNone(comm->config.graphUsageMode),
                                          &comm->sharedRes->deviceStream,
                                          /*concurrent=*/false, &deviceStream), ret, fail);
    CUDACHECKGOTO(cudaMemcpyAsync(&comm->devComm->nRanks, &survivorCount, sizeof(int),
                                  cudaMemcpyHostToDevice, deviceStream), ret, release_stream);
    CUDACHECKGOTO(cudaMemcpyAsync(&comm->devComm->rank, &survivorIndex, sizeof(int),
                                  cudaMemcpyHostToDevice, deviceStream), ret, release_stream);
    for (int c = 0; c < comm->nChannels; c++) {
      struct ncclRing ring = comm->channels[c].ring;
      CUDACHECKGOTO(cudaMemcpyAsync(comm->channels[c].devRingUserRanks, ring.userRanks,
                                    survivorCount*sizeof(int), cudaMemcpyHostToDevice,
                                    deviceStream), ret, release_stream);
      ring.userRanks = comm->channels[c].devRingUserRanks;
      ring.rankToIndex = NULL;
      CUDACHECKGOTO(cudaMemcpyAsync(&devChannels[c].ring, &ring, sizeof(ring),
                                    cudaMemcpyHostToDevice, deviceStream), ret, release_stream);
    }
release_stream:
    { ncclResult_t releaseRet =
        ncclStrongStreamRelease(ncclCudaGraphNone(comm->config.graphUsageMode),
