...
class Primitives<
  ...
  __device__ __forceinline__ void ccldRecordRecv(int nelts, uint64_t waitSpins) {
    ...
    st_relaxed_sys_global(&metrics->lastRecvTimestamp, ts);
    st_relaxed_sys_global(&metrics->lastAnyTimestamp, ts);
  }
  ...
  __device__ __forceinline__ void ccldRecordSend(int nelts) {
    ...
    st_relaxed_sys_global(&metrics->lastSendTimestamp, ts);
    st_relaxed_sys_global(&metrics->lastAnyTimestamp, ts);
  }
  ...
  __device__ __forceinline__ void genericOp(
    ...
    if (tid < nworkers && offset < nelem && !isNetOffload) {
      ...
      do {
        ...
        postPeer<Recv, Send>(0 < workSize, workSize);
        offset += sliceSize;
        slice += 1;
        // Yes, for some template arguments this code will be unreachable.  That's fine.
        // coverity[dead_error_line]
